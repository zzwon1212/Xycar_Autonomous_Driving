...
std::vector<cv::Vec4f> LaneKeepingSystem<PREC>::imageProcess(cv::Mat& frame)
{
    ...
    */
    ...
    square.push_back(cv::Point(0, (int)yOffset+25));
    square.push_back(cv::Point(0, (int)yOffset-25)); 
    square.push_back(cv::Point(frame.cols, (int)yOffset-25)); 
    square.push_back(cv::Point(frame.cols,(int)yOffset+25)); 
    ...
    cv::HoughLinesP(thresframe, lines, 1, CV_PI / 180, 40, 35, 10);
    cv::imshow("mask",mask);
...
{    
    ...
    for (cv::Vec4f line_ : all_lines) {
        ...
        if (abs(slope) < 1) { 
            ...
            if (slope < 0 && x_at_Y_offset <250) {//250 , 295
                left_x_at_Y_offset.push_back(x_at_Y_offset);
            } else if (slope > 0 && x_at_Y_offset >380) {//380 , 345
