    ...
    def run(self):
        for i, batch in enumerate(self.data_loader):
            ...
            with torch.no_grad():
                ...
                # origin 0.4, 0.45 //
                best_box_list = non_max_suppression(output, conf_thres=0.4, iou_thres=0.45)
                ...
                for b in range(num_batch):
                    ...
                    # origin 0.5
                    final_box_list = [bbox for bbox in best_box_list[b] if bbox[4] > 0.0001]
                    # print("final :", final_box_list)

                    if final_box_list is None:
                        continue
                    show_img = input_img[b].detach().cpu().numpy()
                    drawBoxlist(show_img, final_box_list, mode=1, name = "demo/epochs5350/" + str(i) + "_" + str(b))
